...
void Command::privmsg(Message &message)
{
    ...
    //- `PRIVMSG`/ 를 `NOTICE`에 전달할 수 없음 을 나타냅니다 `<channel>`. 이 메시지의 마지막 매개변수에 사용된 텍스트는 다를 수 있습니다.
    //이는 일반적으로 채널이 *[조정되고] 클라이언트가 채널에서 말할 수 있는 권한이 없거나 *[외부 메시지 없음]
    //* 모드가 설정된 채널에 참여하지 않는 등의 채널 모드에 대한 응답으로 전송됩니다.

    //1. 클라이언트가 채널에서 말할 수 없는 권한일 때 이 부분 추가해서 보자 MODE l 때 인 것 같은데 subject에 set이라고 있다. 
    ...
    if (receivers[0][0] == '#') // channel로 통신
    {
        ...
        {
            std::map<std::string, Channel>::iterator iter = channel.find(receivers[i].substr(1, receivers[i].size()));
            if (iter == channel.end())
            {
                no_such_server_402(receivers[i].substr(1, receivers[i].size()));
                return ;
            }
            //2. 클라이언트가 채널에 속해있지 않을 때, ERR_CANNOTSENDTOCHAN (404)
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < receivers.size(); i++)
        {
            std::map<std::string, int>::iterator iter = nicknameToSocketFd.find(receivers[i]);
